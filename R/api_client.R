#' @importFrom httr add_headers accept content timeout
...
#' ApiClient Class
#'
#' Generic API client for OpenAPI client library builds.  OpenAPI generic API
#' client. This client handles the client- server communication, and is
#' invariant across implementations.  Specifics of the methods and models for
#' each application are generated from the OpenAPI Generator templates.
#'
...
ApiClient <- R6::R6Class(
  "ApiClient",
  public = list(
    #' @field base_path base URL of all requests.
    base_path = "https://toppgene.cchmc.org/API",
    #' @field user_agent Default user agent in the HTTP request.
    user_agent = "OpenAPI-Generator/1.0.0/r",
    #' @field default_headers Default headers in the HTTP request.
    ...
    bearer_token = NULL,
    #' @field timeout Default time out (seconds).
    timeout = NULL,
    #' @field retry_status_codes Vector of status codes to retry.
    retry_status_codes = NULL,
    #' @field max_retry_attempts Maximum number of retry attempts for the retry
    #' status codes.
    ...
    deserializeObj = function(obj, return_type, pkg_env) {
      return_obj <- NULL
      primitive_types <-
        c("character", "numeric", "integer", "logical", "complex")
      # To handle the "map" type
      if (startsWith(return_type, "map(")) {
        inner_return_type <-
          regmatches(return_type,
                     regexec(pattern = "map\\((.*)\\)",
                             return_type))[[1]][2]
          ...
          if (!is.null(nrow(obj))) {
            ...
            if (nrow(obj) > 0) {
              for (row in seq_len(nrow(obj))) {
                return_obj[[row]] <-
                  self$deserializeObj(obj[row, , drop = FALSE],
                                      inner_return_type,
                                      pkg_env)
              }
            }
        ...
        # To handle model objects which are not array or map containers.
        # Ex:"Pet"
        ...
        if (!is.null(return_obj$`_discriminator_property_name`)) {
          ...
          if (!is.null(return_obj$`_discriminator_mapping_name`)) {
            data_type <-
              (return_obj$`_discriminator_mapping_name`)[[obj[[data_type]]]]
          } else {
